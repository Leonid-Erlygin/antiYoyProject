...
class State:
    def __init__(self,seed):
        ...
        self.unit_type = np.zeros((self.height, self.width),
                                  "uint8")  # вспомогательный массив, указывающий тип юнита находящегося в данной клетке
        # юнит либо у игрока 1, либо у игрока 2
        self.P1_dict = {"unit1": 4, "unit2": 5, "unit3": 6, "unit4": 7, "player_hexes": 8, "tower1": 9, "tower2": 10,
                        "ambar": 11,
                        "town": 12, "income": 13, "money": 14, "province_index": 15}

        self.P2_dict = {"unit1": 16, "unit2": 17, "unit3": 18, "unit4": 19, "player_hexes": 20, "tower1": 21,
                        "tower2": 22,
        ...
        self.dead_hexes = []  # сюда записываются вражеские гексагоны(единичные провинции), которые в начале хода
    ...
    def getAdjacentHex(self, hexagon, direction):
        ...
        if direction == 4:
            ...
            return hexagon[0] + 1, hexagon[1]
        if direction == 5:
            if hexagon[1] == 0 or (hexagon[0] == self.height - 1 and hexagon[1] % 2 == 1):
                return None
            if hexagon[1] % 2 == 1:
                return hexagon[0] + 1, hexagon[1] - 1
            else:
    ...
    def get_adjacent_friendly_hexes(self, hexagon, player):
        ...
        """
        adjacent = []
        layer = self.P1_dict["player_hexes"] if player == 0 else self.P2_dict["player_hexes"]
        for i in range(6):
            adj = self.getAdjacentHex(hexagon, i)
    ...
    def spawnIsland(self, start_hex, size):
        gen = np.zeros((self.height, self.width), "bool")  # матрица которая будет указывать сгенерировали ли
        # что-то клетка или нет
        gen_potential = np.zeros((self.height, self.width), "uint8")  # Число - потенциал генирации
        ...
        while len(propagation_list) > 0:
            ...
            self.state[hexagon][self.general_dict["gray"]] = 1  # клетка из чёрной становится серой
            self.state[hexagon][self.general_dict["black"]] = 0  # эта клетка больше не чёрная
    ...
    def computeCoordinates(self, hexagon):
        x = self.r * 3 ** 0.5 * hexagon[0] + self.r * ((1 - (-1) ** hexagon[1]) * 3 ** 0.5) / 4
    ...
    def addTrees(self):
        for hexagon in self.activeHexes:
            if self.rs.rand() < 0.1 and self.state[hexagon][self.P1_dict["town"]] == 0 and \
                    self.state[hexagon][self.P2_dict["town"]] == 0:
    ...
    def findGoodPlaceForNewProvince(self, fraction):
        ...
        if fraction == 0:  # Возвращаю случайный серый гексагон
            ...
            moveZoneNumber = (self.state[:, :, self.general_dict["gray"]] == 1) * (-1)  # определён только на серых
    ...
    def spawnProvince(self, spawn_hex, starting_potential):
        ...
        while len(propagationList) > 0:
            ...
            if self.rs.randint(starting_potential) > genPotential[hexagon]:
                continue
            self.state[hexagon][self.P1_dict["player_hexes"] if isPlayer1 else self.P2_dict["player_hexes"]] = 1
            if isPlayer1:
                self.player1_provinces[1].append(hexagon)
                self.state[hexagon][self.P1_dict["province_index"]] = 1  # теперь гексагон в первой провинции
            else:
            ...
            for i in range(6):
                ...
                if adjHex is not None and propagationList.count(adjHex) == 0 \
                        and self.state[adjHex][self.general_dict["black"]] == 0 and \
                        self.state[adjHex][self.general_dict["gray"]] == 1:
        ...
        if isPlayer1:
            for hexagon in self.player1_provinces[1]:
                if self.state[hexagon][self.general_dict["palm"]] == 1 or \
                        self.state[hexagon][self.general_dict["pine"]] == 1:
                    number_of_trees += 1
            income = len(self.player1_provinces[1]) - number_of_trees
            for hexagon in self.player1_provinces[1]:
                self.state[hexagon][self.P1_dict["income"]] = income
                self.state[hexagon][self.P1_dict["money"]] = 10  # Количество денег по умолчанию в начале игры
        else:
    ...
    def spawnProvinces(self):
        ...
        for i in range(quantity):
            for fraction in range(2):
                ...
                self.state[hexagon][self.P1_dict["player_hexes"] if fraction == 0 else self.P2_dict["player_hexes"]] = 1
                self.state[hexagon][
                    self.P1_dict["town"] if fraction == 0 else self.P2_dict["town"]] = 1  # в этом гексагоне есть город
                self.state[hexagon][self.general_dict["gray"]] = 0  # теперь гексагон не серый
                self.spawnProvince(hexagon, fraction + 1)  # игрок 2 имеет приемущество это нужно для баланса
    ...
    def drawGame(self):
        ...
        entity_distribution = {"unit1": [], "unit2": [], "unit3": [], "unit4": [], "tower1": [], "tower2": [],
                               "palm": [],
                               "pine": [],
                               "ambar": [], "town": [], "grave": []}
        ...
        for hexagon in xy:
            ...
            if self.state[hexagon][self.P1_dict["tower1"]] == 1 or self.state[hexagon][self.P2_dict["tower1"]] == 1:
                entity_distribution["tower1"].append(hexagon)
            if self.state[hexagon][self.P1_dict["ambar"]] == 1 or self.state[hexagon][self.P2_dict["ambar"]] == 1:
                entity_distribution["ambar"].append(hexagon)
            if self.state[hexagon][self.P1_dict["tower2"]] == 1 or self.state[hexagon][self.P2_dict["tower2"]] == 1:
                entity_distribution["tower2"].append(hexagon)
            if self.state[hexagon][self.P1_dict["town"]] == 1 or self.state[hexagon][self.P2_dict["town"]] == 1:
                entity_distribution["town"].append(hexagon)

        col = collections.RegularPolyCollection(6, rotation=0,
                                                sizes=((np.pi * self.r ** 2) / sizeScale,), offsets=coordins,
